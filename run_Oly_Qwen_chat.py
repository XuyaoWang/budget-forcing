...
#############################################
    ...
    if not is_chinese:
        results = call_budget_forcing(config,tokenizer,input_list,related_list,stop_think_token="So the final answer is",ignore_str="Wait",num_workers=100,cache_dir = "/aifs4su/hansirui/pcwen_workspace/s1m/cache/Qwen-72_oly")
    else:
        results = call_budget_forcing(config,tokenizer,input_list,related_list,stop_think_token="所以最终答案是",ignore_str="等等",num_workers=100,cache_dir = "/aifs4su/hansirui/pcwen_workspace/s1m/cache/Qwen-72_oly")
    try:
        generated_chunks = []
        ...
        for wait in generated_chunks.keys():
            config_output_path = f"/aifs4su/hansirui/pcwen_workspace/s1m/result/Qwen-72-30168_{subset}-{wait}wait.json"
