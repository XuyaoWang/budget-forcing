...
# 分析结果
model_3b_result_dir = "R1-Onevision-7B-RL-mathv_evaluation_results.json"
model_3b_results = anaylze_results(model_3b_result_dir)
model_3b_results.sort(key=lambda x: x["num_ignore"])

qmodel_7b_result_dir = "R1-Onevision-7B-RL-mathv_evaluation_results.json"
model_7b_results = anaylze_results(qmodel_7b_result_dir)
model_7b_results.sort(key=lambda x: x["num_ignore"])

model_38b_result_dir = "R1-Onevision-7B-RL-mathv_evaluation_results.json"
model_38b_results = anaylze_results(model_38b_result_dir)
model_38b_results.sort(key=lambda x: x["num_ignore"])

model_72b_result_dir = "QVQ-mathv_evaluation_results.json"
model_72b_results = anaylze_results(model_72b_result_dir)
model_72b_results.sort(key=lambda x: x["num_ignore"])

# 创建4个子图，每个子图对应一个模型
models = [
    ("Qwen-2.5-VL-7B_0401_slice-90", model_3b_results),
    ("R1-Onevision-7B-RL", model_7b_results),
    ("Qwen-2.5-VL-7B_0401_slice-150", model_38b_results),
    ("Qwen/QVQ-72B-Preview", model_72b_results)
]

# 设置绘图参数
fig = plt.figure(figsize=(20, 5))  # 一行四个子图的宽高比
plt.rcParams.update({
    'font.size': 12,
    'axes.titlesize': 14,
    'axes.labelsize': 12
...
# 添加单个x轴标签在底部
fig.text(0.5, 0.04, 'Budget Forcing Times (Math Vision)', ha='center', fontweight='bold')
