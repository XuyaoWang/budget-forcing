...
#############################################
    ...
    tokenizer = AutoTokenizer.from_pretrained("/ceph/home/yaodong01/s1-m/Models/R1-Onevision-7B-RL", trust_remote_code=True)
    if not is_chinese:
        results = call_budget_forcing(config,tokenizer,input_list,related_list,stop_think_token="\n</think>\n",ignore_str="\n\nWait",num_workers=200,cache_dir = "./cache_r1-ov-7b")
    else:
        results = call_budget_forcing(config,tokenizer,input_list,related_list,stop_think_token="\n</think>\n",ignore_str="\n\n等等",num_workers=200,cache_dir = "./cache_r1-ov-7b")
    try:
        generated_chunks = {}
        ...
        for wait in generated_chunks.keys():
            config_output_path = f"/ceph/home/yaodong01/s1-m/r1-ov-7b-30168_{subset}-{wait}wait.json"
