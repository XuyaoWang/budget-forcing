...
def eval(model_name, result_prefix):
    ...
    # 为每个wait值评估模型性能
    for wait in range(7):
        file_path = f"{result_prefix}{wait}wait.json"
        ...
        try:
            ...
            for result in tqdm(all_results, desc=f"Processing results with wait={wait}"):
                ...
                # 获取token长度
                thinking = result.get('my_model_thinking', '')
...
eval("Skywork/Skywork-R1V-38B", "/home/hansirui_2nd/pcwen_workspace/s1m_assemble/result/R1V-mathv-30168-")
