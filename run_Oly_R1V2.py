...
#############################################
    ...
    tokenizer = AutoTokenizer.from_pretrained("Skywork/Skywork-R1V2-38B", trust_remote_code=True)
    if not is_chinese:
        results = call_budget_forcing(config,tokenizer,input_list,related_list,stop_think_token="\n</think>\n\n",ignore_str="\n\nWait",num_workers=50,cache_dir = "cache/r1v2_oly")
    else:
        results = call_budget_forcing(config,tokenizer,input_list,related_list,stop_think_token="\n</think>\n\n",ignore_str="\n\n等等",num_workers=50,cache_dir = "cache/r1v2_oly")
    try:
        generated_chunks = {}
        ...
        for wait in generated_chunks.keys():
            config_output_path = f"result/R1V2-30168_{subset}-{wait}wait.json"
